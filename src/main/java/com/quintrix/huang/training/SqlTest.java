...
public class SqlTest {
  ...
  public static void SqlQuery() {
    ...
    try {
      ...
      String query =
          "SELECT employee_name, dept_name " + "FROM employees " + "CROSS JOIN departments; ";
      // + "WHERE (employees.department_id=departments.department_id);";
      // make the query using the provided arg
      try (Statement statem = conn.createStatement();
          ResultSet results = statem.executeQuery(query);) {
        while (results.next()) {
          System.out.println("employee name: " + results.getString("employee_name")
              + ", from department: " + results.getString("dept_name"));
        }
      } catch (SQLException e) {
        System.out.println("A sql error has occured");
        e.printStackTrace();
